 ...
 * @returns Parsed configuration with optional nonInteractive flag
 * @throws {Error} If any argument validation fails
 */
export function parseArgs(): Partial<CLIConfig> & { nonInteractive?: boolean } {
  const args = process.argv.slice(2);
  const config: Partial<CLIConfig> & { nonInteractive?: boolean } = {};
